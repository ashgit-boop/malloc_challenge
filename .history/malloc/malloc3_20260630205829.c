...
void my_add_to_free_list(my_metadata_t *metadata) {
  ...
  my_metadata_t *old_metadata;

  next_metadata = metadata->next; 
  prev_metadata = metadata;

  // 右側マージ // 多分このfor文の一番右側の条件がセグフォの犯人?
  for(tmp_metadata = next_metadata; tmp_metadata!= &my_heap[idx].dummy;tmp_metadata = next_metadata){ // リストでつながっているmetadataを順に追っていく
    next_metadata = tmp_metadata->next;
    // 右隣のマージできそうなものが見つかった
    if(tmp_metadata == (my_metadata_t*)((char*)metadata + sizeof(*tmp_metadata) + metadata->size)){ // 今リスト上で確認しているメタデータが、メモリ上でmetadataの右隣にある空き領域のメタデータだったら
      //sprintf("found\n");
      *old_metadata = *metadata;
      metadata->size = metadata->size + tmp_metadata->size; // 左側のmetadataのサイズを更新 // これのサイズを変えたら別のidxになるかもだから入れ直さなきゃ..?
      //printf("before remove\n");
      my_remove_from_free_list(tmp_metadata,prev_metadata); // 右側にあるメタデータtmpをfree_listから削除
      my_add_to_free_list(metadata); // sizeが変わったmetadataをfree_list_binに入れ直す
      my_remove_from_free_list(old_metadata,NULL); // metadataはリストの先頭に入れられるはずだからprevはNULL
      //printf("after remove\n");
