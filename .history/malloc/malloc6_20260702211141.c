...
//
//my_heap_t my_heap;
my_heap_t my_heap[10]; // free list binを作成0~4の添え字になること想定(size/1000を添え字にして分けてみる)
...
void my_add_to_free_list(my_metadata_t *metadata) {
 ...
 printf("after if in my_add\n");
 printf("my_heap[idx]:%p\n",my_heap[idx]);
...
void my_remove_from_free_list(my_metadata_t *metadata, my_metadata_t *prev) {
  ...
  if (prev) { // もし消すやつが先頭じゃなくてprevがいれば
    ...
    printf("my_heap[idx].free_head:%p\n",my_heap[idx].free_head);
  ...
  metadata->previous = NULL;
  metadata->next = NULL;
  printf("finiched remove\n");
}

// 右結合をする
my_metadata_t* right_merge(my_metadata_t *metadata){ // 引数はfreeしたメタデータ
...
my_metadata_t *left_merge(my_metadata_t *metadata){
    ...
    printf("before if(left_metadata -> is_free == true) \n");
    printf("left_metadata2 : %p\n",left_metadata);
    if(left_metadata -> is_free == true){ // もしmetadataの左隣の領域が空き領域だったら、その領域をリスト上で探して空き領域リストに加える
        
        // 領域を統合
        printf("left_metadata->size : %ld\n",left_metadata->size);
        ...
        printf("after size change\n");
        
        // サイズを更新したleft_metadataを入れ直す
        left_metadata->right = metadata->right; // これがNULLらしい
        printf("after left_metadata->right = metadata->right;\n");
         
        printf("metadata->previous:%p\n",metadata->previous);
        ...
        if(left_metadata->left != NULL && left_metadata -> left -> is_free == true){ 
            printf("left_metadata->right : %p\n",left_metadata->right); // これがNULL
            printf("left_metadata : %p\n",left_metadata);
...
void my_free(void *ptr) {
  ...
  right_metadata = right_merge(metadata); // 右結合
  left_metadata = left_merge(right_metadata); // 左結合
  printf("left_metadata finished \n");
